...
# PHENOTYPE are saved in the h5 element that has the same name as "phenos_v"
# <phenos_v>$row_header$sample_ID = sample ID of ind in phenotype
# <phenos_v>$col_header$phenotype_ID = phenotype names
# <phenos_v>$col_header$covariatesUsed = covariates used for each phenotype; commasep string
# <phenos_v>$matrix = actual matrix with values
...
class SocialData():
    ...
    def __init__(self, in_file=None, phenos_version = None, covs_version=None, cage_version=None, dam_version = None, GRM_version = None, sex_version = None, subset = None, chrom = None):
    ...
    def load(self):
        ...
        # get sex_cov - NB: saved in 'sex_cov' element in h5
        if self.sex_version is None:
            self.sex_cov = None
        else:
            self.sex_cov = f['sex_cov'][self.sex_version]['array'].asstr()[:] 
            self.sex_cov_ID = f['sex_cov'][self.sex_version]['sample_ID'].asstr()[:] #self
    ...
    def get_trait(self, selected_pheno, selected_pheno_MT = None):  #col,col_MT = None):
        ...
        if isinstance(selected_pheno_MT, int):
            ...
            if selected_pheno_MT == "male_female_pheno": # we are in sexvariate, selected pheno is only one
                col_MT = col # only one phenotype # NB: this coul be true also in bivariate where pheno1=pheno2
            else:
        ...
        if col == col_MT and selected_pheno_MT == "male_female_pheno" and self.sex_version is not None: # in sexvariate
            self.trait = "".join([self.trait, "_sex1"])
            self.trait_MT = "".join([self.trait_MT, "_sex2"])
    ...
    def get_data(self, selected_pheno, selected_pheno_MT = None): #col,col_MT = None):
        ...
        col = cols['col']
        col_MT = cols['col_MT'] # None in univariate
        
        self.trait = cols['trait'] 
        self.trait_MT = cols['trait_MT'] # None in univariate
        #mf_mask = False # This is for later

        if col_MT is None:
            self.pheno = self.all_pheno[:,[col]] #using array as index permits keeping the dimensionality kind of
        elif col == col_MT and selected_pheno_MT == "male_female_pheno" and self.sex_version is not None: 
        # we are in sexvariate, need to assign -999 to males for female pheno and viceversa
            # first order sex_cov_ID and sex_cov so that they match pheno_ID
            Imatch = np.nonzero(self.pheno_ID[:,np.newaxis]==self.sex_cov_ID)
            #(self.pheno_ID[Imatch[0]] == self.sex_cov_ID[Imatch[1]]).all()
            #True
            self.sex_cov_ID = self.sex_cov_ID[Imatch[1]]
            self.sex_cov = self.sex_cov[Imatch[1]]
            
            self.pheno_ID = self.pheno_ID[Imatch[0]]
            self.all_pheno = self.all_pheno[Imatch[0],:]
            #(self.pheno_ID==self.sex_cov_ID).all()
            #True
            #cage and kinship now have no missing values and are matched - IDs are in cage_all_ID and kinship_all_ID

            np.unique(self.sex_cov, return_counts = True) # checking how many of each
            # then create mask
            mf_mask = self.sex_cov == "1" # selecting sex = 1 
            #np.unique(mf_mask, return_counts = True)
            
            # then assign -999 to males/females in female/male phenotype
            ### FOR TESTING: creating the phenotype as male pheno + female pheno
            ## test_pheno_m = self.all_pheno[(self.all_pheno[:,[col]] != -999).flatten(), [col]] # .shape 951 
            ## test_pheno_f = self.all_pheno[(self.all_pheno[:,[col+1]] != -999).flatten(), [col+1]] # .shape 918
            ## test_pheno = np.concatenate([test_pheno_m, test_pheno_f], 0).reshape(-1,1) # .shape (1869, 1)
            ## or_pheno = self.all_pheno[:,[col]] # keeping to check 
            ## or_pheno1 = self.all_pheno[:,[col+1]] # keeping to check 
            ## self.all_pheno[:,[col]] = test_pheno
            
            #pdb.set_trace()
            pheno_m = self.all_pheno[:,[col]] 
            pheno_m[np.invert(mf_mask)] = -999 # check: (pheno_m == or_pheno).all() # True
            pheno_f = self.all_pheno[:,[col]] 
            pheno_f[mf_mask] = -999 # check: (pheno_f == or_pheno1).all() # True
            
            self.pheno = np.concatenate([pheno_m, pheno_f], 0) # check: (np.concatenate([pheno_m, pheno_f], 0) == np.concatenate([or_pheno, or_pheno1], 0)).all() # True
            # self.pheno.shape (2N,1); self.pheno_ID.shape (N,)
        ...
        if self.all_covs2use is None:
            ...
            if col_MT is not None: #should only go there is there are covs in the study
                ...
                # TOCHECK if works correctly in sexvariate
                # given col_MT == col it should work ok; NB don't need to set to missing 
