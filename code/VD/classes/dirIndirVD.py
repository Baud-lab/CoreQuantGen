...
from classes.partcorrCov import PartCorrCov # new class to constrain one of corr to 0 when bivariate DGE-IGE model
from classes.lowrankCov_mt import LowRankCovMT # new class to constrain one of corr to 1 when bivariate DGE-IGE model
...
class DirIndirVD():
    ...
    def parseNmore(self, pheno, pheno_ID, covs, covs_ID, covariates_names, kinship_all, kinship_all_ID, cage_all, cage_all_ID, maternal_all, maternal_all_ID, independent_covs,standardize_pheno, subset_IDs,cageEffect, maternalEffect, IEE, subset_on_cage, std_genos, bivariate):
        ...
        ##uCage = ['3827398', '3850313', '3855817', '3866800', '3882417', '3882469', '3882473', '3888760', '3888771', '3888774', 
        #         '3895208', '3903369', '3903409', '3908655', '3914735', '3921408', '3926369', '3930468', '3930502', '3930510']
        #idx_keep = np.concatenate([np.where(cage_all==uCage[i])[0] for i in range(len(uCage))])
        #mask = np.zeros(cage_all.shape,dtype=bool)
        #mask[idx_keep] = True
        #cage_all[~mask]='NA'
        #print('selected cages: ', np.unique(cage_all[cage_all!='NA']))
    ...
    def VD(self, DGE, IGE, IEE, cageEffect,maternalEffect,vc_init, vc_init_type, SimplifNonIdableEnvs, bivariate):
        ...
        #pdb.set_trace()
        #vc_init_type = "lowrank_corr_Ad1d2" # for testing
        if DGE and not IGE: # DGE only
            ...
            if bivariate is False: # univariate
                ...
                elif "null_corr" in vc_init_type:
                    #vc_init_type = "null_corr_Ad1d2"
                    #arrange = [0,1,2,3] # NOT IN USE
                    if "corr_Ad1d2" in vc_init_type: #vc_init_type == "null_corr_Ad1d2": # or could do, if "corr_Ad1d2" in vc_init_type:
                        corr_zero = [1,0]
                    elif "corr_Ad1s1" in vc_init_type: #vc_init_type == "null_corr_Ad1s1":
                        corr_zero = [2,0]
                    elif "corr_Ad1s2" in vc_init_type: #vc_init_type == "null_corr_Ad1s2":
                        corr_zero = [3,0]
                    ## elif "corr_Ad2s1" in vc_init_type: #vc_init_type == "null_corr_Ad2s1": # rearranged D2,D1,S2,S1 4x4
                    ##     corr_zero = [3,0]
                    ##     arrange = [1,0,3,2]
                    ## elif "corr_Ad2s2" in vc_init_type: #vc_init_type == "null_corr_Ad2s2": # rearranged D2,D1,S2,S1 4x4
                    ##     corr_zero = [2,0]
                    ##     arrange = [1,0,3,2]
                    ## elif "corr_As1s2" in vc_init_type: #vc_init_type == "null_corr_As1s2": # rearranged S1,S2,D1,D2 4x4
                    ##     corr_zero = [1,0]
                    ##     arrange = [2,3,0,1]
                    #else:
                    #    sys.exit("the corr in "+vc_init_type+" is not known")
                    #pdb.set_trace()
                    self._genoCov = DirIndirCovMT(self.kinship,Z,kinship_all=self.kinship_all,kinship_cross=self.kinship_cross, Iok = self.Iok_cov, corr_zero=corr_zero)
                elif "lowrank_corr" in vc_init_type:
                    if "corr_Ad1d2" in vc_init_type:
                        print("this has never been tested")
                        idxs = None
                    elif "corr_Ad1s1" in vc_init_type:
                        print("this has never been tested")
                        idxs = [0,2,1,3] # d1, s1, d2, s2
                    elif "corr_Ad1s2" in vc_init_type:
                        print("this has never been tested")
                        idxs = [0,3,1,2] # d1, s2, d2, s1
                    elif "corr_Ad2s1" in vc_init_type:
                        print("this has never been tested")
                        idxs = [1,2,0,3] # d2, s1, d1, s2
                    elif "corr_Ad2s2" in vc_init_type:
                        print("this has never been tested")
                        idxs = [1,3,0,2] # d2, s2, d1, s1
                    elif "corr_As1s2" in vc_init_type:
                        idxs = [2,3,0,1] # s1, s2, d1, d2
                    #pdb.set_trace()
                    self._genoCov = DirIndirCovMT(self.kinship,Z,kinship_all=self.kinship_all,kinship_cross=self.kinship_cross, Iok = self.Iok_cov, lowrank=True, idxs = idxs)
                else:
                    sys.exit(vc_init_type+" not implemented yet for MT covariance")
    ...
    def optimize(self, vc_init, vc_init_type, bivariate): #, seed):
        ...
        if vc_init_type == 'lowrank' or vc_init_type == 'diagonal': # univar only
            assert vc_init is not None, 'need an vc_init to set covariance, run alternative model first'
            if bivariate is False:
        ...
        elif vc_init_type is not None and ("null_corr" in vc_init_type or "lowrank_corr" in vc_init_type): # If is None doesn't give an error whne testing for null_corr in vc_init_type
            #pdb.set_trace()
            assert bivariate, 'this function is for bivariate only'
            #assert DGE and IGE, 'for now available only when all DGE and IGE'
            #assert vc_init is not None, 'need an initiation covariance'
            
            ## TODO: have to double check the below
            ##       check that getInterParams() is correct
            if self._genoCov is not None:
                self._genoCov.C.setCovariance(vc_init['genoCov_K']) #[arrange,:][:,arrange]) #vc_init['genoCov_K']) #CHECK THIS 
                print("genoCov: \n", vc_init['genoCov_K'])
                
            self._envCov.C.setCovariance(vc_init['envCov_K']) 
            print("envCov: \n", vc_init['envCov_K'])
            if self._cageCov is not None:
                self._cageCov.C.setCovariance(vc_init['cageCov_K'])
                print("cageCov: \n", vc_init['cageCov_K'])
            if self._maternalCov is not None:
                self._maternalCov.C.setCovariance(vc_init['maternalCov_K'])
                print("maternalCov: \n", vc_init['maternalCov_K'])
    ...
    def build_output(self, calc_ste, DGE, IGE, IEE, cageEffect, maternalEffect, SimplifNonIdableEnvs, bivariate):
        ...
        R['LML']  = self._gp.LML() # log-marginal-likelihood
