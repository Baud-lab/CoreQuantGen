...
def parse_None(value):
    """Convert string 'None' to Python None, otherwise return the original value."""
    if value == "None":
        return None
    return value
...
parser.add_argument('--grm_v', help= 'kinship type, i.e. name of subgroup of `GRM` in .h5', required=True)
...
# social_data and dirIndirVD need to be in in "classes" subfolder
from classes.social_data import SocialData # if modifying this file, always update the latest version as here
from classes.dirIndirVD import DirIndirVD 
import pdb
import h5py
import re
import csv
...
if __name__=='__main__':
    ...
    elif m == "sexvariate" or m == "sex":
        model = "sex"
        assert sex_version is not None, 'Need the information about sex, store in `sex_cov` in .h5'
        print("model is SEXVARIATE: ", model)
    else:
        sys.exit("Need to know which model to use: " + "'"+ m +"' " + "is not valid" + "\n==> Use 'uni' / 'univariate' OR 'bi' / 'bivariate' OR 'sex' / 'sexvariate'")
    ...
    if model == "bi":
        ...
        selected_pheno = args['pheno'] # if int, NOT YET in python 0-indexing
        selected_pheno = int_or_str(selected_pheno) # aquiring selected pheno
        selected_pheno_MT = "male_female_pheno" # this is a tag to know we are in sexvariate
    ...
    if analysis_type == 'null_covars_LOCO':
        ...
        elif model == "bi" or model =="sex":
            sys.exit("can't generate LOCO variances with bivariate or sexvariate model yet")
    ...
    if analysis_type == 'VD': # 
        ...
        if corr0 is not None: # == 'corr_Ad1d2': # if there is corr0, add it to the filename and that is going to be it
            ...
            if corr0 == "corr_Ad1d2":
                 assert model == "bi" and DGE is not None, " ".join(["need bivariate and DGE to constrain", corr0, "to 0 or 1"])
            if constr == 0:
                VD_outfile_name="_".join([VD_outfile_name,corr0,'zero']) # add at the end of the normal filename "_corr*_zero" 
            elif constr == 1:
                VD_outfile_name="_".join([VD_outfile_name,corr0,'one']) # add at the end of the normal filename "_corr*_one"
            else:
                sys.exit("corr should be constrained to 0 or 1")
        ...
        if not os.path.isfile(VD_outfile_pickle): # pickle not existing, meaning we are running for the first time
            ...
            while try_nb_VD < 6: # SEED_test: increase the number of tries until one works/doens't work, the seed will be printed so that you can reproduce it
                ...
                #pdb.set_trace()
                print("Starting VD of alternative model")
                print("in vc")
                #pdb.set_trace()
        ...
        if corr0 is not None: 
            ...
            my_vc_init = vc_covs
            if constr == 0:
                if corr0 == "corr_Ad1d2" and IGE is None: #or corr0 == "corr_Ad1s1": #TODO check if this want
                    my_vc_init_type = 'diagonal' # this might change depending on the corr?
                elif "corr_Ad1" in corr0 and IGE is not None:
                    my_vc_init_type = "".join("null_"+corr0)
                    calc_ste_VD=False
            elif constr == 1:
                my_vc_init_type = "".join("lowrank_"+corr0)
                calc_ste_VD=False
            else:
                sys.exit("covariance class with corr not including Ad1 not implemented yet. Torna-hi mes tard.")
            ...
            try_nb_VD = 1
            while try_nb_VD < 6:
                print("Starting VD of with corr "+corr0+" constrained to "+str(constr))
                print('in vc_lr')
                ...
                if "optim_error" in dirIndir_out.keys():
                    assert dirIndir_out['optim_error'] is not None, "optimization error is None, when expected to be something"
                    if try_nb_VD < 6:
                        print("Warning: optimization with corr ", corr0," constrained to ",str(constr)," was not successful, gonna try for the ", try_nb_VD , " /5 time")
                    else:
                        print("Warning: optimization with corr ", corr0," constrained to ",str(constr)," was not successful after 5 tries")
                        break
                    sid = sid+1   # SEED_test: change number for next test
                else:
                    print("Optimization with corr ", corr0," constrained to ",str(constr)," successfull!")
                    break
